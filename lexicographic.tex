...
%			\Output{$\Gamma_{\operatorname{LMC}}$} 
			...
			\For{$e \in \GraphE$ in decreasing order}{
				...
				\alert<+|handout:0>{$r_1 \leftarrow $\FindSet{$v_1$} \\
								    ...
								    $c_1 \leftarrow $\FindSet{$\widetilde{\tau_1}$} \\
									$c_2 \leftarrow $\FindSet{$\widetilde{\tau_2}$}
			    } \\
				\eIf{\alert<+|handout:0>{$\{ r_1, r_2\} = \{ c_1, c_2 \}$}}{
...
\begin{frame}{Optim homologous relative cycle}
